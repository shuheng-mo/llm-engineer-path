...
长对话会超出模型的上下文窗口，导致模型无法访问之前的对话内容，这时候就需要对短期记忆进行管理了。常见的管理策略有：

| 策略 | 说明 | 适用场景 |
| --- | --- | --- |
| 修剪消息 | 保留最近 N 条消息 | 快速简单，但会丢失早期信息 |
| 删除消息 | 从状态中永久删除特定消息 | 清理敏感信息或无关消息 |
| 总结消息 | 总结历史记录并替换 | 保留完整上下文，推荐用于复杂任务 |

参考代码在`stage2-advanced/04-langgraph/04_module_persistence/03_message_strategies`下。

1. 修剪消息是最简单的方式，直接保留最近的 N 条消息，就像滑动窗口一样，适合对话不太长或者对早期信息依赖不大的场景。
2. 或者超出就删除，针对的是隐私保护或者状态管理、节省数据库存储的场景。和策略1的区别仅仅在于，1是存了但是选择性的不让模型看，2的话是直接简单粗暴的删除。
...
1. 上下文管理压力减少，每个agent只看到和自己相关的上下文，避免信息过载。
2. 分布式开发，多个agent可以由不同的团队独立开发，最后通过定义好的接口进行集成。
3. 并行执行效率高，可以同时执行多个子任务。

这里我们学习5大MAS核心架构模式：

1. **子代理模式**(中心化控制)
2. **Handoff交接模式**(去中心化控制)
3. **Skills技能模式**(按需加载模式)
4. **Router路由模式**(分类路由再聚合)
5. **自定义工作流模式**(完全自定义设计)

对于如何选择这几种MAS设计模式，有一个简单的决策树：

- agents需要和用户对话，Handoff+Skills，如果不需要顺序约束，Skills。

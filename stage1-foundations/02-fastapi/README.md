...
| `ch06-dependency-injection/` | Depends · yield 生命周期 · APIRouter · 分层架构 | 5 个子项目 (`apirouter_intro/`, `apirouter_full/`, `apirouter_advanced/`, `layered_arch/` + 单文件示例) |
...
第一次运行会在 `ch05-orm/` 下生成 `books.db`（已加入 `.gitignore`）。
...
- `ch06/06_session_dep_pattern.py` 和 `ch06/07_use_session_in_route.py` 引用了 `ch05-orm/orm/` 下的 `models.py` 和 `database.py`（教学连续性）。
...
| ch06 `apirouter_intro/routers/book.py` 合并了表 34 + 表 35，补 `from typing import Dict` | 原 docx 这两块代码分别给出，且未导入 Dict |
